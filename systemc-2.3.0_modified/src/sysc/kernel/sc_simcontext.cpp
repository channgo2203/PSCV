...
//CHIMP
void
sc_simcontext::attach_observer(mon_observer* obs) {
  observer = obs;
}

//CHIMP
mon_observer*
sc_simcontext::get_observer() {
  return observer;
}

//random scheduler
void 
sc_simcontext::enable_random_scheduler( bool b) {
	ran_enable = b;
}

bool 
sc_simcontext::is_random_enabled() {
	return ran_enable;
}

void 
sc_simcontext::set_seed( unsigned int seed_) {
	random_seed = 	seed_;
}

unsigned int 
sc_simcontext::get_seed() {
   return random_seed;
}
...
sc_simcontext::crunch( bool once )
{
	...
	while( true ) 
	{
		...
		{
		    //CHIMP
                    // run_process() invokes the method, so at this point
                    // the method failed to run. But we will anyway execute the monitors.
                    if (observer != 0)
                      { observer->method_suspend(); }
		    goto out;
		}
                //CHIMP
		// run_process() invokes the method, so at this point
                // the method has suspended itself
                if (observer != 0)
                    { observer->method_suspend(); }
	...
	// remove finally dead zombies:
        ...
        //CHIMP
        if (observer != 0){ observer->evaluate_end(); }
	...
	//CHIMP
        if (observer != 0){ observer->update_end(); }

	m_execution_phase = phase_notify;

        //CHIMP
        if (observer != 0){ observer->deltanotify_begin(); }
	...
	if ( size != 0 )
	{
        ...
        //CHIMP
        if (observer != 0){ observer->deltanotify_end(); }
...
#   endif
    ...
    //CHIMP
    if (observer != 0) {observer->init_begin();}
    ...
    //CHIMP
    if (observer != 0) {observer->init_update_begin();}

    m_prim_channel_registry->perform_update();

    //CHIMP
    if (observer != 0) {observer->init_update_end();}

    m_execution_phase = phase_notify;

    //CHIMP
    if (observer != 0) {observer->init_delta_notification_begin();}
    ...
    //CHIMP
    if (observer != 0) {observer->init_delta_notification_end();}

    //CHIMP
    // These two if-statements can be combined. But it is better
    // to keep them separate in case the standard changes in the future.
    if (observer != 0) {observer->init_end();}
}
...
sc_simcontext::simulate( const sc_time& duration )
{
	...
	do {
	    ...
	    // PROCESS TIMED NOTIFICATIONS AT THE CURRENT TIME
            ...
            //CHIMP
            if (observer != 0) {observer->timednotify_begin();}
        ...
        // CHIMP
        if (observer != 0) {observer->timednotify_end();}
...
// Set the random seed for controlled randomization
// Call this function to initialize the randomized scheduler
void 
sc_set_random_seed()
{
	// get simulation context
	sc_simcontext* m_simc = sc_get_curr_simcontext();
	int sim_ok = m_simc->sim_status();
	
	if (sim_ok != SC_SIM_OK) {
		return;
	}
	
	sc_status sim_status = m_simc->get_status();
	bool isvalid = (sim_status != SC_UNITIALIZED) && 
	               !(sc_is_running(m_simc)) && 
	               !(sc_start_of_simulation_invoked());
	//simulator is ok, is initialized, and is not running
	if (isvalid) {  
		//enable the randomized scheduler
		m_simc->enable_random_scheduler(true); 
		//seed the random generator
		unsigned int a_seed = time(NULL) * getpid();
	   srand(a_seed);
		m_simc->set_seed(a_seed);
	}
	else {
		SC_REPORT_WARNING( SC_ID_NOT_IMPLEMENTED_,
		       "No effect: sc_set_random needs to be called before any sc_start call" );
	}
}
